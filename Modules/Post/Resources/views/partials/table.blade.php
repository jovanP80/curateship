        ...
        <tbody class="int-table__body js-int-table__body">
            @foreach($posts as $key => $post)
              ...
              <tr class="int-table__row">
                ...
                <td class="int-table__cell" aria-controls="modal-edit-article">
                  <a href="{{ url('admin/posts/edit/' . $post->id) }}" data-update-url="{{ url('admin/posts/update/' . $post->id) }}" class="modal-trigger-edit-post" aria-controls="modal-edit-post" role="button">{{ $post->title }}</a>
                </td>
                <td class="int-table__cell">{{ $post->getPostByUser->email ?? '' }}</td>
                <td class="int-table__cell">{{ date('F m, Y H:i:s',strtotime($post->created_at)) }}</td>
      ...
      <ul class="pagination__list flex flex-wrap gap-xxxs">
        ...
        <li>
          <span class="pagination__jumper flex items-center">
            <input aria-label="Page number" class="form-control" type="text" id="pageNumber" name="pageNumber" value="{{ $posts->currentPage() ?? 0 }}">
            <em>of {{ $posts->lastPage() ?? 0 }}</em>
          </span>
        </li>
